<h1 align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:000000,50:1a0033,100:000000&height=120&section=header" width="100%"/>
</h1>

<h1 align="center">Hi there, I'm Javin! 👋</h1>

<h3 align="center">🌙 Night-owl developer | 🎮 Gamer | 🎹 Pianist | 🚀 Building things that matter</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=1000&color=FF00FF,00FFFF&center=true&vCenter=true&width=600&lines=16-Year-Old+Self-Taught+Developer;Class+11+Science+Student+(PCM%2BCS);Building+Discord+Bots+%26+Web+Apps;Future+Software+Engineer+%F0%9F%9A%80" alt="Typing SVG" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Javin27&label=Profile%20Views&color=ff00ff&style=for-the-badge" alt="Profile views" />
  <img src="https://img.shields.io/github/followers/Javin27?label=Followers&style=for-the-badge&color=00ffff&logo=github&logoColor=white" alt="GitHub followers" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/STATUS-ONLINE-39FF14?style=for-the-badge&logo=statuspage&logoColor=white&labelColor=000000" />
</p>

---

## 🧠 About Me

```python
class Javin:
    def __init__(self):
        self.name = "Javin Rathod"
        self.age = 16
        self.role = "Student | Self-Taught Developer"
        self.education = "Class 11 — Physics, Chemistry, Math, Computer Science"
        self.long_term_goal = "Crack JEE 🎯 | Become a Software Engineer 💻"
        self.currently_building = ["Discord Bots", "Web Interfaces", "Automation Tools"]
        self.interests = ["Coding", "Music Production", "Gaming", "UI/UX Design"]

    def say_hi(self):
        print("Always learning, always building. Let's connect! 👋")
```

- 🔭 Currently building automation tools and web apps while juggling JEE prep
- 🌱 Deepening my CS fundamentals and exploring advanced algorithms
- 🎮 When I'm not coding, you'll probably find me deep in a Minecraft world or mixing a track
- ⚡ Fun fact: I run on curiosity, caffeine-free energy, and way too many open tabs

---

## 🛠️ Technologies & Tools

**Languages**

<p align="left">
  <img src="https://img.shields.io/badge/Python-FF00FF?style=for-the-badge&logo=python&logoColor=00FFFF&labelColor=000000" />
  <img src="https://img.shields.io/badge/JavaScript-FF00FF?style=for-the-badge&logo=javascript&logoColor=00FFFF&labelColor=000000" />
  <img src="https://img.shields.io/badge/HTML5-FF00FF?style=for-the-badge&logo=html5&logoColor=00FFFF&labelColor=000000" />
  <img src="https://img.shields.io/badge/CSS3-FF00FF?style=for-the-badge&logo=css3&logoColor=00FFFF&labelColor=000000" />
</p>

**Automation & Frameworks**

<p align="left">
  <img src="https://img.shields.io/badge/Discord.py-00FFFF?style=for-the-badge&logo=discord&logoColor=FF00FF&labelColor=000000" />
  <img src="https://img.shields.io/badge/GitHub_Actions-00FFFF?style=for-the-badge&logo=github-actions&logoColor=FF00FF&labelColor=000000" />
</p>

**Tools & Platforms**

<p align="left">
  <img src="https://img.shields.io/badge/VS_Code-39FF14?style=for-the-badge&logo=visual-studio-code&logoColor=000000&labelColor=000000" />
  <img src="https://img.shields.io/badge/Git-39FF14?style=for-the-badge&logo=git&logoColor=000000&labelColor=000000" />
  <img src="https://img.shields.io/badge/GitHub-39FF14?style=for-the-badge&logo=github&logoColor=000000&labelColor=000000" />
  <img src="https://img.shields.io/badge/GitHub_Pages-39FF14?style=for-the-badge&logo=github-pages&logoColor=000000&labelColor=000000" />
</p>

**Currently Learning**

<p align="left">
  <img src="https://img.shields.io/badge/Data_Structures_%26_Algorithms-9600FF?style=for-the-badge&labelColor=000000" />
  <img src="https://img.shields.io/badge/CS_Fundamentals-9600FF?style=for-the-badge&labelColor=000000" />
  <img src="https://img.shields.io/badge/JEE_Prep-9600FF?style=for-the-badge&labelColor=000000" />
</p>

---

## 🚀 Projects

### 🎮 Free Game Giveaway Bot
A Discord bot that automatically tracks and instantly alerts users about **free game giveaways** on Steam and Epic Games — fully automated via **GitHub Actions** cron scheduling, no server babysitting required.

`Python` · `Discord.py` · `GitHub Actions` · `REST APIs`

### 🛍️ Craftroom E-Commerce Interface
A fully responsive, sleek frontend homepage built for a family **craft and wellness brand** — designed from scratch and deployed on GitHub Pages.

`HTML5` · `CSS3` · `JavaScript` · `GitHub Pages`

### 🕹️ Gaming Scripting & Server Optimization
Hands-on experience with **server optimization, scripting, and modding** for sandbox multiplayer environments — tuning performance and building custom gameplay features.

`Scripting` · `Server Administration` · `Modding`

> 💡 More projects coming soon — open to collaboration and contributions!

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Javin27&show_icons=true&theme=synthwave&hide_border=true&bg_color=000000&title_color=FF00FF&icon_color=00FFFF&text_color=c9d1d9" alt="GitHub Stats" height="170"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Javin27&layout=compact&theme=synthwave&hide_border=true&bg_color=000000&title_color=FF00FF&text_color=c9d1d9" alt="Top Languages" height="170"/>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Javin27&theme=synthwave&hide_border=true&background=000000&ring=FF00FF&fire=00FFFF&currStreakLabel=00FFFF" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Javin27&theme=synthwave&hide_border=true&bg_color=000000&color=00FFFF&line=FF00FF&point=ffffff" alt="Contribution Graph" width="100%"/>
</p>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Javin27/Javin27/output/github-contribution-grid-snake-dark.svg">
  <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/Javin27/Javin27/output/github-contribution-grid-snake.svg">
</picture>

> 📝 To enable the snake animation above, add the [GitHub Action by Platane](https://github.com/Platane/snk) to your profile repo's workflows.

---

## 🎵 Beyond Coding

- 🎹 **Music** — A decent pianist with a growing interest in digital audio production, MIDI synthesis, and mixing
- 🎮 **Gaming** — Avid fan of open-world sandbox games (Minecraft Java/Bedrock), strategy, and multiplayer titles
- 🎨 **Design & Branding** — Hands-on with graphic layouts, UI/UX branding, and digital operations for creative business ventures
- 🎌 **Culture** — Big anime fan, slowly learning Japanese 🇯🇵

---

## 🎯 Goals

- [ ] Crack JEE 🎯
- [ ] Master advanced DSA & algorithms
- [ ] Become a full-stack developer
- [ ] Contribute to open source
- [ ] Land a role at a top tech company
- [ ] Keep building things that matter

---

## 📫 Connect with Me

<p align="center">
  <a href="https://github.com/Javin27"><img src="https://img.shields.io/badge/GitHub-FF00FF?style=for-the-badge&logo=github&logoColor=00FFFF&labelColor=000000" /></a>
  <a href="#"><img src="https://img.shields.io/badge/Discord-00FFFF?style=for-the-badge&logo=discord&logoColor=FF00FF&labelColor=000000" /></a>
  <a href="#"><img src="https://img.shields.io/badge/Portfolio-39FF14?style=for-the-badge&logo=vercel&logoColor=000000&labelColor=000000" /></a>
</p>

<p align="center">
  <i>⭐️ Add your Discord tag and portfolio link above — replace the "#" hrefs with your real links!</i>
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:000000,50:1a0033,100:000000&height=100&section=footer" width="100%"/>
</p>

<p align="center">
  <i>"Keep learning. Keep improving. Build things that matter." ✨</i>
</p>
